    ...
    def __init__(self, output_dim,
        ...
        if len(ln_init) == 2 and float in [type(l) for l in ln_init]:
            ln_init = [lambda shape,
                       name=None: K.variable(l, dtype='float32')*K.ones(shape, name=name) for l in ln_init]
        if len(mi_init) == 3 and float in [type(m) for m in mi_init]:
            mi_init = [lambda shape,
                       name=None: K.variable(m, dtype='float32')*K.ones(shape, name=name) for m in mi_init]
