...
def make_sequence_example(inputs, labels):
  """Returns a SequenceExample for the given input and label.
  Args:
  ...
  """
  input_features = [
      tf.train.Feature(float_list=tf.train.FloatList(value=[input_])) for input_ in inputs.tolist()]
  label_features = [
      tf.train.Feature(int64_list=tf.train.Int64List(value=[label])) for label in labels.tolist()]
  feature_list = {
